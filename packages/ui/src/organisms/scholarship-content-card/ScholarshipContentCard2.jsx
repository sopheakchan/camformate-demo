...
import { avatar1, avatarImage } from '../../../assets'
...
const ScholarshipContentCard2 = ({ data }) => {
  ...
  const timeLeft = `${
    daysLeft * 1 < 0 ? 'Out of date' : daysLeft + ' days left'
  }`
  ...
  const removeFav = () => {
    axios
      .delete(`${BASE_URL}/users/${uid}/favorites`, {
        ...
        data: { _id: data?.id, type: 'scholarships' },
      })
      .then(response => {
        setFav(!fav)
      })
  ...
  return (
    <div className="relative w-[300px] h-[400px] rounded-2xl bg-white shadow-lg flex flex-col justify-between">
      {/* share modal */}
      <ShareModal
        showModal={shareModalShow}
        onClose={() => setShareModalShow(false)}
        shareUrl={`https://camformant.com/scholarships/${data?.id}`}
        title={`Camformant - ${
      ...
      >
        ...
        <Image
          src={
            data?.attributes?.user_profile !== 'none'
              ? data?.attributes.user_profile
              : avatarImage
          }
          alt="avatar"
          width={50}
          height={50}
          className="absolute bottom-[25px] bg-white left-[10px] aspect-square object-cover rounded-full border-2 border-primary"
        />
      </div>
      ...
      <div className="p-2 flex flex-col gap-2">
        ...
        <p className="text-gray font-semibold">
          {data?.attributes?.slot} applicants
        </p>
      </div>
      ...
      <div className="flex flex-col justify-between p-2">
        <div className="p-2 rounded-lg flex items-center">
          ...
          <span className="text-danger">
            ...
            {data?.attributes?.price?.type === 'Percent %' ? '%' : '$'}{' '}
        ...
        <div className="w-[95%] p-2 rounded-lg flex items-center">
          ...
          <span>&nbsp;{data?.attributes?.location}</span>
        </div>
      </div>
...
export default ScholarshipContentCard2
